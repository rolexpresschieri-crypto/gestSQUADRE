-- Inoltro automatico push TOC → operatori su allarme volontario.
-- Eseguire su Supabase dopo schema_v1.sql e squad_alarms_request_types.sql.

-- Matrice tipologia allarme → destinatario (le X del foglio operativo).
create table if not exists alarm_notify_routing (
  alarm_type text not null check (
    alarm_type in ('sanitario', 'security', 'vvf', 'altro')
  ),
  admin_code text not null,
  is_enabled boolean not null default true,
  created_at timestamptz not null default now(),
  primary key (alarm_type, admin_code)
);

create index if not exists alarm_notify_routing_admin_idx
  on alarm_notify_routing (admin_code);

-- Token FCM telefono operatore TOC (admin_code da toc_admins).
...
-- Seed matrice (globale, tutti i campi).
insert into alarm_notify_routing (alarm_type, admin_code) values
  ('sanitario', 'GT_01_AN'),
  ('sanitario', 'GT_01_EN'),
  ('sanitario', 'GT_01_LDP'),
  ('sanitario', 'GT_01_UN'),
  ...
  ('altro', 'GT_01_LDP'),
  ('altro', 'GT_01_UN')
on conflict (alarm_type, admin_code) do nothing;
