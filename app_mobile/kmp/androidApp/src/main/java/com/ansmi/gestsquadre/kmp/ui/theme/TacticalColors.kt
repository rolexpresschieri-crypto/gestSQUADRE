package com.ansmi.gestsquadre.kmp.ui.theme

import androidx.compose.ui.graphics.Color

val BrandTint = Color(0xFFA60035)
val BrandBase = Color(0xFF0A0607)
const val BrandBackgroundAlpha = 0.48f

val TacticalFrame = Color(0xFF14295D)
val TacticalGreen = Color(0xFF079B42)
val TacticalYellow = Color(0xFFE0BE3A)
val TacticalRed = Color(0xFFC62828)
val TacticalOrange = Color(0xFFFF9A05)
val TacticalNavy = Color(0xFF1A3066)
val TacticalMuted = Color(0xFF8A9AAA)
val TacticalDisabled = Color(0xFF4A5568)
